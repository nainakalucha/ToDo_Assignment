...
namespace Adform_ToDo.BL
{
    ...
    public class ToDoListManager : IToDoListManager
    {
        ...
        {
            return await _toDoListDal.GetToDoListById(toDoListId, userId);
        }
        ...
        {
            return await _toDoListDal.GetAllToDoLists(userId);
        }
        ...
        {
            List<ToDoListDto> todoLists = await _toDoListDal.GetAllToDoLists(userId);
            if (!string.IsNullOrWhiteSpace(paginationParams.SearchString))
            {
                todoLists = todoLists.Where(p => p.Description.ToLower().Contains(paginationParams.SearchString.ToLower())).ToList();
            }
        ...
        {
            return await _toDoListDal.CreateToDoList(createToDoListDto);
        }
        ...
        {
            return await _toDoListDal.UpdateToDoList(updateToDoListDto);
        }
        ...
        {
            return await _toDoListDal.DeleteToDoList(id, userId);
        }
        ...
        {
            return await _toDoListDal.AssignLabelToList(assignLabelToListDto);
        }
    }
}
